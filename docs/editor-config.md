...
### Перевірка налаштувань у редакторі

Після зміни .editorconfig потрібно перевірити, що редактор застосовує правила до відповідних типів файлів.

Для Python-файлів використовується відступ у 4 пробіли, а для інших файлів базове значення становить 2 пробіли.

Для Markdown вимкнено автоматичне видалення кінцевих пробілів, оскільки вони можуть використовуватися для явного переносу рядка.

Таким чином, налаштування перевіряються не лише за вмістом .editorconfig, а й за фактичною поведінкою редактора.

### Перевірка .gitignore

Для перевірки конкретного файла можна використати:
...
git check-ignore -v test.tar.gz
```

Команда показує правило .gitignore, через яке Git ігнорує вказаний файл.
